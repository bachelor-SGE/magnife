...
// УДАЛЯЕМ use Redis;  <-- (это встроенный PHP-класс без connection())
// ДОБАВЛЯЕМ вместо него фасад:
...
class Controller extends BaseController
{
    ...
    {
        // Используем фасад Redis от Laravel:
    ...
    public function changeBalance(Request $r){
        ...
        $games_on = Crash::where('user_id', $user->id)->count() 
                   + $games_on_s 
                   + Wheel::where('user_id', $user->id)->count() 
                   + X100::where('user_id', $user->id)->count() 
                   + $games_on 
                   + $games_on_c;
    ...
    public function winterStart(Request $r){
        ...
        $prize = [5,5,5,7,7,7,10,10,15,15,20,25,30,100,200,300];
        shuffle($prize);
    ...
    public function addDemoBalance(Request $r){
        ...
        if($addbalance < 1){ 
            return response(['success' => false, 'mess' => 'Минимальная сумма - 1р' ]);
        }
        if($addbalance > 25000){
            return response(['success' => false, 'mess' => 'Максимальная сумма - 25000р' ]);
        }
    ...
    public function refsChange(Request $r){
        ...
        if(\Cache::has('action.user.' . $user->id)){
            return response(['success' => false, 'mess' => 'Подождите перед предыдущим действием!' ]);
        }
        \Cache::put('action.user.' . $user->id, '', 1);

        if($user->balance_ref < 5){
            return response(['success' => false, 'mess' => 'Минимум 5' ]);
        }
    ...
    public function promoCreate(Request $r){
        ...
        if($user->deps < $dep_createpromo){
            return response([
                'success' => false,
                'mess' => 'Сумма пополнений должна быть больше чем '.$dep_createpromo
            ]);
        }
    ...
    public function transferGo(Request $r){
        ...
        if(!(\Cache::has('user.'.$my_user->id.'.historyBalance'))){
            \Cache::put('user.'.$my_user->id.'.historyBalance', '[]');
        }
    ...
    public function getHistory(Request $r){
        ...
        if($type == 'deps'){
            ...
            $history = Withdraw::where('user_id', $user->id)
                               ->orderBy('id', 'desc')
                               ->take(20)->get();
        }
    ...
    public function bonusRef(){
        ...
        if ($refs < 10){
            return response(['success' => false, 'mess' => 'У вас меньше 10 рефералов' ]);
        }
    ...
    public function bonusCheckTg()
    {
        ...
        if ($bonus_2 != 2){
            return response(['success' => false, 'mess' => 'Ваш аккаунт не привязан' ]);
        }
    ...
    public function bonusGetTg(Request $request){
        ...
        $bonus_2 = $user->bonus_2;
        if ($bonus_2 == 1){
            return response(['success' => false, 'mess' => 'Вы уже получали бонус' ]);
        }
        if ($bonus_2 == 0){
            return response(['success' => false, 'mess' => 'Привяжите свой аккаунт TG', 'modal' => 'tg' ]);
        }
        $setting = Setting::first();
    ...
    public static function isMessages($id) {
        ...
        try {
            $arr = json_decode(file_get_contents(
                "https://api.vk.com/method/messages.getHistory?access_token=".$grtok."&group_id=".$grid."&user_id=".$id."&v=5.103"
            ), true);

            return isset($arr['error'])
                ? false
                : ($arr['response']['count'] == 0 ? false : true);
        } catch (\Exception $e) {
            return false;
        }
    }
    ...
    public function bonusGetVk(Request $request){
        ...
        $vk1 = json_decode(file_get_contents(
            "https://api.vk.com/method/users.get?user_ids={$domainvk}&access_token=".$grtok."&v=5.131"
        ));
        ...
        $resp = file_get_contents(
            "https://api.vk.com/method/groups.isMember?group_id=".$grid."&user_id={$vk1}&access_token=".$grtok."&v=5.131"
        );
        $data = json_decode($resp, true);
        if($data['response'] != '1'){
            return response([
                'success' => false,
                'link' => $setting->group_link,
                'mess' => 'Подпишитесь на нашу группу!'
            ]);
        }

        if(!$this->isMessages($user->vk_id)){
            return response()->json([
                'success' => false,
                'mess' => 'Напишите "+" в личные сообщения группы'
            ]);
        }
    ...
    public function bonusGet(Request $request){
        ...
        $user_deps = Payment::where('status', 1)
        ...
        if($user_deps < 0){
            return response([
                'success' => false,
                'mess' => 'Для получения бонуса требуется минимальная сумма пополнений за 7 дней - 0р. (У вас '.$user_deps.'р)'
            ]);
        }
        ...
        $w__time = $hours.':'.$minutes.':'.$sec;

        if($f_time < 43200){
            return response([
                'success' => false,
                'mess' => "Ожидайте еще {$w__time}"
            ]);
        }
